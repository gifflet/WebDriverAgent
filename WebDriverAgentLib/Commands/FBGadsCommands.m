...
// Production broadcast host (h264-broadcast-extension, display name "GADSBroadcast"): hosts an
// RPSystemBroadcastPickerView pre-targeted at the appex, auto-pressed onAppear.
...
// Foreground the GADSBroadcast host so its picker fires the iOS broadcast sheet, then drive the
// sheet to select the target row (body: {"target": "<display-name>"}, default "GADSBroadcast").
+ (id<FBResponsePayload>)handleAudioPrepare:(FBRouteRequest *)request
{
  // Single broadcast host extension; the picker may still list other apps' extensions, so the
  // flow below selects this one specifically by its display name.
  ...
  // Autotap the system sheet: select the GADSBroadcast row before the action button, since iOS
  // spawns the appex only when that radio is selected (preferredExtension is just a hint).
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    // Start-action labels (broadcast inactive — normal flow). The action predicate also matches
    // stopLabels so Path B detects the sheet even when already broadcasting (Phase 1.5 handles it).
    NSArray<NSString *> *startActionLabels = @[
      @"Iniciar Gravação",
      @"Start Recording",
      @"Iniciar Transmissão",
      @"Iniciar Difusão",
    ...
    NSArray<NSString *> *actionButtonLabels = [startActionLabels arrayByAddingObjectsFromArray:stopLabels];
    ...
    // Phase 1 — locate an entry point: try Path A (broadcast button), else the record button
    // for Phase 2 (swipe). Retry up to maxAttempts to absorb sheet animation latency.
    for (NSUInteger attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      if (broadcastBtn.exists && broadcastBtn.isHittable) {
        // A hittable broadcast button only proves SOME extension is selected, not the target.
        // Confirm via exact value match ("1"/"Selected"/"Selecionado") before tapping.
    ...
    // Phase 1.5 — if already broadcasting (action button is "Stop Broadcast"), just press Home
    // to dismiss the picker; swiping/tapping would toggle the broadcast OFF.
    ...
    // Phase 2 — the picker is a horizontal carousel; a row is only isHittable when it's the
    // selected radio. Swipe up to maxHorizontalSwipes times until the target is hittable, then tap.
    ...
    // Pick a swipe target: prefer the pager indicator, else the Fotos row (page 1 anchor),
    // else springboard itself.
    ...
    // Safety gate: only tap the action button once accessibility confirms the target is the
    // selected radio — tapping the wrong radio spawns the wrong extension.
